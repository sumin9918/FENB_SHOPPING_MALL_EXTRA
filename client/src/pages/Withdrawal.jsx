...
const Withdrawal = () => {
  ...
  return (
    <Stack pb="2rem" px="0.8rem" spacing="3.2rem" w="100%">
      ...
      <Stack spacing="3rem">
        <Title>회원탈퇴에 앞서 아래 내용을 반드시 확인해 주세요.</Title>
        <Stack p="2.4rem" spacing="3rem" sx={{ border: `1px solid ${colors.gray[5]}` }} w="100%">
          <Stack spacing={0}>
            <Group pb="1.2rem" sx={{ flexWrap: 'nowrap' }}>
              <BsCaretDownSquare size="2.4rem" style={{ minWidth: '2.4rem' }} />
              <Title>486을 탈퇴하면 회원 정보 및 서비스 이용 기록이 삭제됩니다.</Title>
            </Group>
            <DotText>
          ...
          <Stack spacing={0}>
            <Group pb="1.2rem" sx={{ flexWrap: 'nowrap' }}>
              <BsCaretDownSquare size="2.4rem" style={{ minWidth: '2.4rem' }} />
              <Title>관련 법령 및 내부 기준에 따라 별도 보관하는 경우에는 일부 정보가 보관될 수 있습니다.</Title>
            </Group>
            <Title fz="1.8rem" py="0.8rem">
          ...
          <Stack spacing={0}>
            <Group pb="1.2rem" sx={{ flexWrap: 'nowrap' }}>
              <BsCaretDownSquare size="2.4rem" style={{ minWidth: '2.4rem' }} />
              <Title>486 탈퇴가 제한된 경우에는 아래 내용을 참고하시기 바랍니다.</Title>
            </Group>
        ...
        <Checkbox
          checked={checked}
          label={<Title>회원탈퇴 안내를 모두 확인하였으며 탈퇴에 동의합니다.</Title>}
          size="2.8rem"
          sx={{ cursor: 'pointer' }}
...
const DotText = ({ children }) => (
  <Group fz="1.8rem" py="0.2rem" sx={{ flexWrap: 'nowrap' }}>
    <RxDotFilled size="2rem" style={{ minWidth: '2rem' }} />
    <Text>{children}</Text>
  </Group>
);
